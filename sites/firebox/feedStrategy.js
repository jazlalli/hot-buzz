...
var FireboxConsumer = function FireboxConsumer() {};
FireboxConsumer.prototype = Object.create(defaultStrategy.prototype);

FireboxConsumer.prototype.execute = function (feed, callback) {
...
var mapData = function mapData(products) {
	...
	products.forEach(function (p) {
		...
		product.title = dataUtils.cleanProductName(p.product_name.text);
		product.image = p.product.src;
		product.site = 'firebox';
...
var saveProducts = function saveProducts(products) {
	products.forEach(function (product) {
		Product.upsert(product, function (err) {
			if (err) {
				console.log('FIREBOX PRODUCT SAVE ERROR');
			} else {
...
module.exports = FireboxConsumer;